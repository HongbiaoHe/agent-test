...
> ⚠️ **阶段前置（强制）**：执行本阶段前，必须先 `read_file` 读取 `references/treatment.md`（limit=1000）。在成功读取它之前，**禁止输出任何创意方向或创意方案文档**。
...
> ⚠️ **阶段前置（强制）**：执行本阶段前，必须先 `read_file` 读取 `references/shot-language.md`（limit=1000，重点 Part 2 画风锚定词库）。在成功读取它之前，**禁止输出任何画风锚定词或提示词**。
...
> ⚠️ **阶段前置（强制）**：执行本阶段前，必须先 `read_file` 读取 `references/pre-production.md`（limit=1000）；生成资产图提示词的格式还需读取 `references/delivery.md` Part 1。在成功读取前，**禁止输出任何资产图提示词**。
...
> ⚠️ **阶段前置（强制）**：执行本阶段前，必须先 `read_file` 读取 `references/storyboard.md`（limit=1000，多宫格写作规范与产品电影化系统）。在成功读取它之前，**禁止输出任何多宫格或视频提示词**。
