...
describe('createMediaTools', () => {
  ...
  it('generate_image：createGeneration 抛 BusinessException 时工具返回含 error 的 JSON（不 throw）', async () => {
    const svc = {
      createGeneration: jest.fn().mockRejectedValue(
        new BusinessException(ErrorCodes.MEDIA_REF_INVALID, HttpStatus.BAD_REQUEST),
      ),
    } as unknown as MediaService;
    const [imageTool] = createMediaTools(svc, ctx);
  ...
  it('generate_image：createGeneration 抛普通 Error 时仍 throw', async () => {
    const svc = {
      createGeneration: jest.fn().mockRejectedValue(new Error('db connection failed')),
    } as unknown as MediaService;
    const [imageTool] = createMediaTools(svc, ctx);

    await expect(imageTool.invoke({ prompt: '测试' })).rejects.toThrow('db connection failed');
  });
